    ...
    if request.method == 'POST' and data_dict != {}:
        if 'dob' in request.form:
            ...
            if not calculate_metrics():
                return render_template('upload_file.html', msg="Not enough data to calculate metrics !!")
            return render_template('status_table.html', all_metrics=all_metrics, bmi=bmi)
...
def calculate_metrics():
    ...
    activities_data['Minutes Fairly Active'] = activities_data['Minutes Fairly Active'].apply(clean_col_data)
    activities_data['Minutes Very Active'] = activities_data['Minutes Very Active'].apply(clean_col_data)
    activities_data['Minutes Lightly Active'] = activities_data['Minutes Lightly Active'].apply(clean_col_data)
    activities_data['Minutes Sedentary'] = activities_data['Minutes Sedentary'].apply(clean_col_data)
    activities_data.dropna(inplace=True)

    activities_data = activities_data.loc[(activities_data['Minutes Sedentary'] + activities_data[
        'Minutes Lightly Active'] + activities_data['Minutes Fairly Active'] + activities_data[
                                               'Minutes Very Active']) >= 600]
    if len(activities_data) < 3:
        return False

    activities_data['Steps'] = activities_data['Steps'].apply(clean_col_data)

    activities_data['Minutes Active'] = activities_data['Minutes Fairly Active'] + activities_data[
        'Minutes Very Active']
    total_minutes_per_week = activities_data['Minutes Active'].sum()
    all_metrics["physical_activity_average_steps"] = round(activities_data['Steps'].mean())
    ...
    create_week_bar_graph("week_bar_physical_activity.png", activities_data, 'Date', 'Minutes Active',
                          'Moderate-to-Vigorous Physical Activity (min)')
    create_single_bar_graph("single_bar_physical_activity.png", total_minutes_per_week,
                            "Moderate-to-Vigorous Physical Activity (min/week)", float('inf'), threshold, [threshold],
                            'min')
    # print("ACTIVITIES DATA: ")
    # print(activities_data)

    # calculate metrics for sedentary time
    activities_data['Hours Sedentary'] = activities_data['Minutes Sedentary'].apply(lambda x: x / 60)
    ...
    all_metrics["sedentary_time_average"] = round(activities_data['Minutes Sedentary'].mean(), 2)
    all_metrics["patiend_average_st_hours_day"] = round(all_metrics["sedentary_time_average"] / 60, 1)
    ...
    create_week_bar_graph_alter("week_bar_sedentary_time.png", activities_data, 'Date', 'Hours Sedentary',
                          'Sedentary time (hours)')
    create_single_bar_graph("single_bar_sedentary_time.png", round(all_metrics["sedentary_time_average"] / 60, 2),
                            "Sedentary time (hours/day)", all_metrics["sedentary_time_guideline"], 0,
                            [all_metrics["sedentary_time_guideline"]], 'hrs')

    # calculate metrics for sleep time
    sleep_data = data_dict["Sleep"]
    print("DATA DICT SLEEP: ", sleep_data['Start Time'])
    sleep_data['Minutes Awake'] = sleep_data['Minutes Awake'].apply(lambda x: int(x.replace('"', '')))
    sleep_data['Hours Awake'] = sleep_data['Minutes Awake'].apply(lambda x: round(x / 60, 2))
    sleep_data['Time in Bed'] = sleep_data['Time in Bed'].apply(lambda x: int(x.replace('"', '')))
    sleep_data['Hours Bed'] = sleep_data['Time in Bed'].apply(lambda x: round(x / 60, 2))
    sleep_data['Minutes Asleep'] = sleep_data['Minutes Asleep'].apply(lambda x: int(x.replace('"', '')))
    sleep_data['Hours Asleep'] = sleep_data['Minutes Asleep'].apply(lambda x: round(x / 60, 2))
    ...
    all_metrics["sleep_time_average"] = calculate_average_time(sleep_data['Start Time'], 'start')
    all_metrics["wake_time_average"] = calculate_average_time(sleep_data['End Time'], 'end')
    ...
    create_week_bar_graph("week_bar_sleep_time.png", sleep_data, 'Date', 'Hours Asleep', 'Sleep time (hours)')
    create_single_bar_graph("single_bar_sleep_time.png", all_metrics["sleep_time_asleep"], "Sleep time (hours/day)", 9,
                            7, [7, 9], 'hrs')
...
def create_single_bar_graph(img_save_path, data, y_label, upper_threshold, lower_threshold, threshold_lines, text_post):
    ...
    if data >= lower_threshold and data <= upper_threshold:
        ...
        ax.bar(x, [data - upper_threshold], 0.5, color="r", bottom=upper_threshold, align='center')
    else:
    ...
    plt.text(0.38, data, str(data) + ' ' + text_post, fontsize=18, bbox=dict(facecolor='white'))
    img_path = os.path.join(app.config['UPLOAD_FOLDER'], img_save_path)
    if os.path.exists(img_path):
        os.remove(img_path)
    fig.savefig(img_path)
...
def parse_csv_file(file):
    ...
    for data_type in data_dict:
        data_dict[data_type] = pd.DataFrame(data_dict[data_type][1:], columns=data_dict[data_type][0])


def clean_line_data(line_data):
    line_data = re.sub(r'(".*?")', lambda match: match.group(1).replace(',', ''), line_data)
